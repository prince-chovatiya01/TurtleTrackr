...
# Strategy simulation function with the new 5% down condition
def simulate_turtle_strategy(df, investment_amount, lookback_period, profit_target, min_buy_gap):
    open_orders = []  # Each order: (buy_date, buy_price)
    total_profit = 0
    buy_signals = 0
    ...
    for i in range(lookback_period, len(df)):
        ...
        # Compute current average buy price if there are open orders
        avg_buy_price = None
        if open_orders:
            avg_buy_price = sum([order[1] for order in open_orders]) / len(open_orders)

        # Check if we can buy:
        # (a) breakout AND
        # (b) min_buy_gap satisfied AND
        # (c) if holding, price is 5% down from avg buy price
        can_buy = True
        if open_orders:
            last_buy_date = open_orders[-1][0]
            # Enforce minimum days gap between buys
            if current_date < (last_buy_date + timedelta(days=min_buy_gap)):
                can_buy = False
            # Enforce 5% down from avg buy price
            elif not (current_close <= avg_buy_price * 0.95):
                can_buy = False

        # If no open orders, only breakout+gap matters (gap check will pass because open_orders is empty)
        if can_buy and df['High'].iloc[i] >= period_high:
        ...
        # Sell condition: if open orders exist and current close >= profit_target * average buy price.
        if open_orders:
            # Recompute avg_buy_price here in case it was None
            avg_buy_price = sum([order[1] for order in open_orders]) / len(open_orders)
            if current_close >= profit_target * avg_buy_price:
                ...
                open_orders = []  # Clear all open positions
...
with st.expander("About the Turtle Trading Strategy"):
    ...
    **Key Components:**
    ...
    - **Minimum Buy Gap:** Once a buy occurs, no new buys are triggered until this many days have passed.
    - **Position Sizing:** A fixed investment amount per trade.
    - **5% Down Rule:** When you’re holding shares, a new buy is only triggered if the current close is at least 5% below your average holding price.
