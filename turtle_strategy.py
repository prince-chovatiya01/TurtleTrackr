...
with st.sidebar:
    ...
    if mode == "Normal Mode":
        ...
        # Fix investment amount during optimization (you can adjust if you wish)
        investment_amount = 100_000

        # Ranges for optimization, with slider + numeric input for min/max
        col1, col2, col3 = st.columns([2, 1, 1])
        with col1:
            lookback_range = st.slider("Lookback Period Range (Days)", 5, 100, (30, 70))
        with col2:
            lookback_min = st.number_input(
                "Min", min_value=5, max_value=100, value=lookback_range[0]
            )
        with col3:
            lookback_max = st.number_input(
                "Max", min_value=5, max_value=100, value=lookback_range[1]
            )
        lookback_range = (lookback_min, lookback_max)
        ...
        with col2:
            profit_min = st.number_input(
                "Min", min_value=1.0, max_value=1.5,
                value=profit_target_range[0], step=0.01, format="%.2f"
            )
        with col3:
            profit_max = st.number_input(
                "Max", min_value=1.0, max_value=1.5,
                value=profit_target_range[1], step=0.01, format="%.2f"
            )
        profit_target_range = (profit_min, profit_max)
        ...
        with col2:
            gap_min = st.number_input(
                "Min", min_value=0, max_value=60, value=min_buy_gap_range[0]
            )
        with col3:
            gap_max = st.number_input(
                "Max", min_value=0, max_value=60, value=min_buy_gap_range[1]
            )
        min_buy_gap_range = (gap_min, gap_max)
...
# -------------------------------------------------------------
# Strategy simulation function with explicit float() casts
# -------------------------------------------------------------
def simulate_turtle_strategy(df, investment_amount, lookback_period, profit_target, min_buy_gap):
    open_orders = []  # list of tuples: (buy_date, buy_price)
    total_profit = 0.0
    buy_signals = 0
    sell_signals = 0
    trade_details = []       # list of dicts for each closed trade
    investment_tracker = []  # list of { 'date': timestamp, 'investment': float }
    transaction_log = []     # detailed log of each buy/sell event

    df = df.sort_index()

    for i in range(lookback_period, len(df)):
        current_date = df.index[i]
        # Highest high over the lookback window (scalar)
        period_high = float(df['High'].iloc[i - lookback_period : i].max())
        current_close = float(df['Close'].iloc[i])

        # Compute current average buy price if open_orders exist
        avg_buy_price = None
        if open_orders:
            avg_buy_price = float(sum([order[1] for order in open_orders]) / len(open_orders))

        # 1) Determine whether we’re allowed to BUY today
        can_buy = True
        if open_orders:
            last_buy_date = open_orders[-1][0]
            # Enforce minimum days between buys
            if current_date < (last_buy_date + timedelta(days=min_buy_gap)):
                can_buy = False
            # Enforce “5% down from average buy price” when already holding
            elif not (current_close <= avg_buy_price * 0.95):
                can_buy = False

        # 2) If breakout AND can_buy, place a new BUY
        #    (Note: df['High'].iloc[i] and period_high are both floats now)
        if can_buy and float(df['High'].iloc[i]) >= period_high:
        ...
        # 3) Track daily investment based on open_orders count
        investment_tracker.append({
            'date': current_date,
            'investment': len(open_orders) * investment_amount
        })

        # 4) Check SELL condition: if holding, compare to profit_target * avg_buy_price
        if open_orders:
            # Recompute avg_buy_price in case it changed
            avg_buy_price = float(sum([order[1] for order in open_orders]) / len(open_orders))
            if float(current_close) >= float(profit_target) * avg_buy_price:
                # Calculate profit per order
                profit_per_order = (current_close / avg_buy_price - 1) * investment_amount
                trade_profit = profit_per_order * len(open_orders)
                total_profit += trade_profit
                sell_signals += len(open_orders)
                ...
                # Clear all open orders after selling
                open_orders = []

    # Determine maximum investment that occurred on any day
    max_investment = 0
    ...
    return {
        ...
        'investment_over_time': (
            pd.DataFrame(investment_tracker).set_index('date') if investment_tracker else pd.DataFrame()
        ),
        'transaction_log': pd.DataFrame(transaction_log) if transaction_log else pd.DataFrame(),
        'max_investment': max_investment
    }
...
def create_investment_chart(ticker, investment_df, trades):
    ...
    # Add vertical dashed red line at each sell date
    for trade in trades:
        fig.add_vline(
...
# -------------------------------------------------------------
# Generate a Base64‐encoded Excel download link containing:
#   • Summary sheet
#   • TradeDetails sheet
#   • AllTransactions sheet
#   • DailyInvestment sheet
# -------------------------------------------------------------
def generate_excel_download_link(results):
    buffer = BytesIO()
    ...
    # Write to Excel workbook in memory
    with pd.ExcelWriter(buffer, engine='xlsxwriter') as writer:
        summary_df.to_excel(writer, sheet_name='Summary', index=False)
        if not trade_df.empty:
            trade_df.to_excel(writer, sheet_name='TradeDetails', index=False)
...
# -------------------------------------------------------------
# Main analysis logic: runs when the “Run Analysis” button is pressed
# -------------------------------------------------------------
if run_analysis:
    results = {}
    ...
    if mode == "Optimization Mode":
            ...
            for lb in lookback_vals:
                for pt in profit_vals:
                    for gap in gap_vals:
                        ...
                        # Objective: maximize (total_profit / (max_investment + 1))
                        if res['max_investment'] > 0 and res['sell_signals'] > 0:
        ...
        # Normal Mode: use the user‐provided lookback_period, profit_target, min_buy_gap
        data = yf.download(ticker, start=start_date, end=end_date, auto_adjust=False)
        if data.empty:
            st.error(f"No data available for {ticker}")
        else:
            ...
            summary_df = pd.DataFrame.from_dict(
                {
                    ticker: {
                        ...
                        'Return (%)': (
                            round(
                                (result['total_profit'] / result['max_investment']) * 100,
                                2
...
with st.expander("About the Turtle Trading Strategy"):
    ...
    **Key Components:**
    - **Lookback Period:** Number of days to determine the highest high.
    - **Profit Target:** Multiplier (e.g. 1.06 means a 6% gain) at which all positions are sold.
    - **Minimum Buy Gap:** After a buy, no new buys until this many days have passed.
    - **Position Sizing:** Fixed investment amount per trade.
    - **5% Down Rule:** If you’re already holding shares, a new buy only triggers if the current close is ≥5% below your average buy price.

    In **Optimization Mode**, the app automatically searches for the lookback period, profit target, 
    and min‐buy‐gap that maximize the ratio of total profit to max investment.
    """)
